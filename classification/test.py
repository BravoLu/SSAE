...
def test(opt):
    ...
    logs = '../logs/classification/%s_%s_%s'%(opt.dataset, opt.target, mode)
    ...
    target_weights = os.path.join('../targets/classification/%s_%s.pth'%(opt.dataset, opt.target))
    target_model = init_model(opt.target, target_weights)
